...
const LINK_ON = "text-aisc-ink opacity-100";
const LINK_OFF = "text-aisc-ink/55 opacity-55 hover:text-aisc-forest hover:opacity-100";
...
export default function ArticleToc({ headings }: { headings: Heading[] }) {
  ...
  return (
    <aside className="lg:col-start-1 lg:sticky lg:top-32">
      <div className="border-aisc-forest border lg:max-h-[calc(100dvh-24rem)] lg:overflow-y-auto">
        <div className="lg:hidden">
          <button
            ...
            onClick={() => setOpen((v) => !v)}
            className="hover:bg-aisc-cream flex w-full items-center justify-between gap-3 px-5 py-4 text-left transition-colors"
          >
            <span className="text-display-3 md:text-display-3-lg">Contenido</span>
          ...
          <div id="article-toc-mobile-list" className={`grid transition-[grid-template-rows] duration-200 motion-reduce:transition-none ${open ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
            <div className="overflow-hidden">
              <nav aria-label="Contenido del artículo" className="px-5 pt-1 pb-5">
        ...
        <div className="hidden flex-col gap-6 p-5 lg:flex">
          <nav aria-label="Contenido del artículo" className="flex flex-col gap-3">
            <h2 className="text-display-3 md:text-display-3-lg">Contenido</h2>
            <List headings={headings} active={active} />
          </nav>
        </div>
      </div>
