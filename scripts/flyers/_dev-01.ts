...
const OUT_FILE = path.join(OUT_DIR, '01-lanzamiento.png');
...
async function renderOnce() {
  ...
  const t0 = Date.now();
  try {
    const modPath = path.join(FLYERS_DIR, '_v3-apart.ts');
    ...
    version++;
    lastError = null;
    console.log(`[v${version}] ${(result.png.length/1024).toFixed(0)}KB in ${Date.now()-t0}ms`);
  } catch (e: any) {
...
for (const f of ['_v3-apart.ts', 'lib.ts']) {
...
<html><head><meta charset="utf-8"><title>01-lanzamiento dev</title>
<style>
...
<body><div class="wrap">
  <div class="bar"><span id="v">v—</span><span id="t">—</span></div>
  <div id="err"></div>
  <img id="img" alt="01-lanzamiento" />
</div>
<script>
let cur = -1;
async function poll() {
...
server.listen(PORT, () => {
  console.log(`dev preview at http://localhost:${PORT}/`);
  console.log('watching _v3-apart.ts and lib.ts — saves trigger re-render');
  renderOnce();
});
