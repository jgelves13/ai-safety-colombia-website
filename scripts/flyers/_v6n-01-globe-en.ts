...
import { buildGlobeDataUrl, GlobeVariant } from './_globes';

export default async function (variant: GlobeVariant = 'A') {
  const W = 1080;
  const H = 1350;

  const logos = await getLogos(130);
  const globe = await buildGlobeDataUrl(variant);
  ...
  return { filename: `01-en-globe-${variant}.png`, png };
}
