...
export default async function (format: FlyerFormat = 'portrait') {
  ...
  const globeSize = isPort ? 320 : 260;
  const globeTop = isPort ? 150 : 130;
  const globeRight = isPort ? -40 : -30;
  ...
  const globe = globeGlobalSouth({
    size: globeSize,
    sphereFill: '#F3E8D0',
    sphereStroke: BRAND.forest,
    sphereStrokeWidth: 4,
    gridStroke: BRAND.forest,
    otherLandFill: BRAND.ink2,
    regions: {
      latam: BRAND.sage,
      africa: BRAND.forest,
      asia: BRAND.yellow,
  ...
  return { filename: `_v5b-01-illustrated-${format}.png`, png };
}
