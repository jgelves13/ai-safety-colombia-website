...
export default async function (format: FlyerFormat = 'portrait') {
      ...
      <div style="display:flex;flex-direction:column;width:${W}px;height:${H}px;padding:${padT}px 80px ${padB}px 80px;justify-content:space-between">
        ...
        <div style="display:flex;flex-direction:column;gap:${blockGap}px;max-width:940px">
          ...
          <div style="display:flex;flex-direction:column;gap:10px">
            <div style="display:flex;font-family:JetBrains Mono;font-size:${labelSz}px;letter-spacing:0.16em;color:${CORAL}">PREMIO LATAM</div>
            <div style="display:flex;font-family:Bricolage Grotesque;font-weight:800;font-size:${heroSz}px;line-height:0.95;letter-spacing:-0.035em;color:${FOREST}">USD 1.000</div>
            <div style="display:flex;font-family:Inter;font-weight:500;font-size:${heroSubSz}px;line-height:1.4;color:#0C0C0C;max-width:680px;margin-top:10px">por equipo, para cada uno de los tres ganadores en América Latina.</div>
          </div>

          <div style="display:flex;width:${dividerW}px;height:${dividerH}px;background:${CORAL};margin-top:6px"></div>

          <div style="display:flex;font-family:Bricolage Grotesque;font-weight:700;font-size:${midSz}px;line-height:1.12;letter-spacing:-0.018em;color:${FOREST};max-width:920px">Pero el dinero no es lo importante.</div>

        </div>

        <div style="display:flex;flex-direction:column;gap:20px">
          <div style="display:flex;align-items:stretch;gap:20px">
            ${step('01', 'Hackathon', 'Tres días resolviendo retos curados por investigadores con experiencia.')}
            ${step('02', 'Apart Lab Fellowship', 'Los ganadores continúan 12 a 24 semanas con Apart, hacia investigación publicable.')}
            ${step('03', 'Vinculación', 'Publicación en conferencias del campo y conexión con organizaciones de seguridad de IA.')}
          </div>
        </div>
  ...
  return { filename: `_va-06-premio-${format}.png`, png };
}
