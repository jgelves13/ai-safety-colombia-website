...
export default async function (format: FlyerFormat = 'portrait') {
  ...
  const speakers = [
    {
      ...
      aff: 'OpenAI',
      mode: 'Remoto',
      topic: 'Robustez adversarial: defender modelos cuando los ataques ya están en circulación.',
      bio: 'Ingeniero de investigación en alineación en OpenAI. Coautor de IH-Challenge y LLM Critics: defensas contra inyección de prompts y modelos que evalúan a otras IAs.',
    },
    {
      accent: YELLOW,
      photo: toroPhoto,
      ...
      mode: 'Presencial',
      topic: 'Lo que Colombia está discutiendo, hoy, sobre armas autónomas letales.',
      bio: 'Representante a la Cámara. Coautor y coordinador para primer debate del PL 368/2025, que prohíbe las armas autónomas letales y regula las semiautónomas.',
    },
  ];
    ...
    <div style="display:flex;width:${W}px;height:${H}px;background:${BRAND.cream};font-family:Inter;position:relative;overflow:hidden">
      ...
      <div style="display:flex;flex-direction:column;width:${W}px;height:${H}px;padding:${padT}px 80px ${padB}px 80px;gap:${headerGap}px">
        ...
        <div style="display:flex;flex-direction:column;gap:12px;max-width:820px">
          <div style="display:flex;font-family:JetBrains Mono;font-size:${kickerSz}px;letter-spacing:0.22em;color:${FOREST};text-transform:uppercase">Día 1 · charlas de apertura</div>
          <div style="display:flex;font-family:Bricolage Grotesque;font-weight:700;font-size:${headingSz}px;line-height:1.0;letter-spacing:-0.024em;color:${FOREST}">Dos miradas para empezar el sprint.</div>
        </div>
  ...
  return { filename: `_va-04-ponentes-${format}.png`, png };
}
